...
| 3 | v0.6.0 Drop 3 | `0.6.0-drop3-shared-job-controls-importing.md` | **ACTIVE — Phases 0, 1 and 2 complete (2026-08-08).** Branch `feature/0.6.0-drop3-shared-job-controls-importing`, start SHA `563df9884497032e19abd4437a0e66584cd9ec12`. Phase 1 added the frozen importing/job-control vocabulary (`shared/importing.py`, `shared/job_control.py`) and 307 tests, and repaired the Phase 0 baseline defect under approved option (a). Phase 2 added the read-only traversal core — natural ordering, broad-root classification, hidden detection, non-following identity capture and `scan_roots` — plus 97 tests; its link-classification risk gate was reached, evidenced and cleared, and `maintenance.py` was **not** refactored. Adopted by no production panel. Phases 3–10 not started; each needs separate explicit maintainer approval. See the recorded start state in Section 15. | Plans 1–2 |
...
| Phase reached | **Phases 0, 1 and 2 complete (2026-08-08).** The branch head carries `shared/importing.py` (contracts **and** the read-only traversal core) and `shared/job_control.py`, plus 404 Plan 3 tests; **1481 collected, 1470 passed, 11 skipped, 1 warning**, theme 17/17, `verify.py` **RESULT: PASS** on three consecutive runs, compile gate and `git diff --check` clean. Six of the eleven skips are Phase 2's, each naming its exact platform limitation. No production panel or launcher adopts the foundation. Phases 3–10 not started |

...
| 3 | v0.6.0 Drop 3 | `0.6.0-drop3-shared-job-controls-importing.md` | **ACTIVE — Phases 0 and 1 complete (2026-08-08).** Branch `feature/0.6.0-drop3-shared-job-controls-importing`, start SHA `563df9884497032e19abd4437a0e66584cd9ec12`. Phase 1 added the frozen importing/job-control vocabulary (`shared/importing.py`, `shared/job_control.py`) and 307 tests, adopted by no production panel, and repaired the Phase 0 baseline defect under approved option (a). Phases 2–10 not started; each needs separate explicit maintainer approval. See the recorded start state in Section 15. | Plans 1–2 |
...
The next action is **Plan 3 Phase 2 — safe natural traversal core**. It is **not started** and requires explicit maintainer approval before any work begins. Its own risk gate applies: if `maintenance.is_link` cannot safely classify an importing case, stop and report the narrow evidence-backed extraction needed rather than refactoring maintenance or output-path services. Plan 3 runs on its own branch from the verified `origin/master`, must not be developed on the Drop 1 or Drop 2 feature branch, must not reopen Decisions 1–55, must not adopt the new foundation into any production panel, and must not begin Plan 4.

*Superseded (kept for the record): between Phases 0 and 1 this section named **Plan 3 Phase 1 — pure contracts and compatibility boundaries** as the next action. Earlier still, after Plan 2's closeout and before the merge, it said there was no active implementation document, named Plan 2 integration review as the only next action, and described Plan 3 as undrafted and unstarted; before that closeout it named `md-instructions/0.6.0-drop2-config-output-maintenance-foundation.md` as the active document.*
...
| Phase reached | **Phases 0 and 1 complete (2026-08-08).** Phase 1 head carries `shared/importing.py` and `shared/job_control.py` plus 307 tests; **1381 collected, 1376 passed, 5 skipped, 1 warning**, theme 17/17, `verify.py` **RESULT: PASS**, compile gate and `git diff --check` clean. No production panel or launcher adopts the foundation. Phases 2–10 not started |

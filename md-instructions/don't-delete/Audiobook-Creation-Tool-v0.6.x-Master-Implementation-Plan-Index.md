...
**Cite integration commits, never a live branch tip (added 2026-09-03).** A plan's merge commit is a permanent fact and is the right thing to record here. The current tip of `master` is not: writing “`master` now points to X” into a permanent record is **self-invalidating**, because merging the very document that says it advances `master` past X. That is not hypothetical — it happened when PR #7 landed and immediately falsified the sentence PR #7 had just introduced. So record **stable plan/PR integration anchors** in permanent prose, and treat an exact branch tip as orientation evidence to be re-queried from Git at the moment it matters.
...
`feature/0.6.4-m4b-maker-metadata-editor`. **v0.6.4 is therefore confirmed merged** (the independent
read-only integration-readiness recheck named below ran and returned READY). `feature/0.6.5-tts-quality-refactor`
was created from that verified `origin/master`. Separately, the maintainer opened **v0.6.5 as the TTS
Quality Refinement plan** (`0.6.5-tts-quality-refactor.md`), not as Plan 9 — Plan 9's full scope is
preserved and remains approved future work, now recorded as **DISPLACED — FUTURE ALLOCATION
UNASSIGNED** (see Section 7's Plan 9 note and Section 15). Everything else in the paragraph below —
the Phase 15 retirement of both temporary plans, the Windows/macOS acceptance record, and version
identity `0.6.2` unreleased — still stands; only its "not merged" clause and its "current action is
the recheck" clause are superseded by this paragraph.
...
| 6 | v0.6.3 Drop 1 | `0.6.3-drop1-shared-multi-book-workspace.md` | **COMPLETE and MERGED into `master` through pull request #10** (merge commit `e0bab662b385734807bf264d8f450aebac053dcc`, 2026-09-13, a normal two-parent merge; parent 1 the PRE-PLAN-6 anchor `83a2bfc7de25dbe5a599b48fd73fe306695e490d`, parent 2 the approved feature head `4b988676ac258f4f3b4991aa92c99679a2e6ce33`); branch `feature/0.6.3-drop1-shared-multi-book-workspace` **retained**. **The retained Plan-6 drop file is ELIGIBLE FOR RETIREMENT as of v0.6.4 Phase 14 (2026-09-17)** — every reason it was kept is now satisfied mechanically: the MP3 Tool (v0.6.3), the M4B Maker and the M4B Metadata Editor (both v0.6.4) all import `shared/book_workspace`, `test_plan6_boundaries.PHASE0_PANEL_HASHES` is empty because every consumer panel left it by a proved conversion, and the lasting contracts are promoted to `Decisions.md` (2026-09-12 and 2026-09-17). **It is not deleted**; deletion awaits the maintainer's explicit approval at the v0.6.4 Phase 14 closeout gate. *(Superseded status text, kept for the record: this row previously read* **COMPLETE for this drop's shipped scope; AWAITING MAINTAINER INTEGRATION through pull request #10** (2026-09-13). The shared multi-Book foundation — **Plan 6 Phases 0–8** — is **COMPLETE**. Its focused MP3 continuation (the temporary plan `0.6.3-plan6-mp3-tool-redesign.md`, **Phases 1–13**, which rebuilt the MP3 Tool on that foundation) is **COMPLETE**, with **Windows acceptance complete** (Phase 11, maintainer, real launcher and real media) and **macOS acceptance complete** (Phase 12, maintainer, real launcher and real media); that focused temporary plan was **retired after its Phase 13 closeout** with the maintainer's explicit approval. Branch `feature/0.6.3-drop1-shared-multi-book-workspace`; the completed feature checkpoint before this reconciliation is **`7d3726f273e47f37acf1dd1202e9e6fd13619e5e`** (`v0.6.3 MP3: retire completed focused plan`). **Pull request #10 is OPEN against `master` and awaiting the maintainer's integration decision — it is NOT merged.** The broader Plan 6 drop `0.6.3-drop1-shared-multi-book-workspace.md` is **retained, not retired**, because its M4B Maker and M4B Metadata Editor adoptions of the shared workspace have **not started and are intentionally unscheduled** under this drop. Version identity remains **`0.6.2`, UNRELEASED**: no `[0.6.2]`/`[0.6.3]` changelog heading, no tag, no release, no package. *(Superseded status text, kept for the record: this row previously read “**DRAFTED / PLANNING COMPLETE; implementation NOT STARTED** (2026-09-07). The drop exists and is the single active temporary implementation drop, on branch `feature/0.6.3-drop1-shared-multi-book-workspace` created from `10a03ed66480d396d65881eea84cb8ae4bfe452e`. Eleven phases (0–10). **No phase has run and no production code exists** — no `shared/book_workspace.py`, no `shared/numbering.py`, no `shared/book_workspace_ui.py`, no test, no panel change. Three architecture decisions were settled by the maintainer at plan creation: one Plan 3 `RunSnapshot` **per book**; `mp3_tools/m4b_numbering.py` **promoted** to `shared/numbering.py` behind a re-export shim under a blocking Converter regression gate; and a **developer-only harness** on the Plan 3 precedent, so the foundation ships adopted by no production panel. **Plan 6 does not close or release v0.6.3** — Plan 7 does. No version bump, tag, release or package. Phase 0 is the next bounded action and needs separate explicit maintainer approval.” — and before that “Planned; not drafted”.)* | Plans 1–3 |
| 7 | v0.6.3 Drop 2 | `0.6.3-drop2-m4b-maker-multi-book.md` *(never created)* | **SUPERSEDED by maintainer ruling on 2026-09-13.** The M4B Maker adoption was not delivered as a separate drop or a separate branch; it was delivered inside **v0.6.4 together with the M4B Metadata Editor** (see the Plan 8 row) and is **COMPLETE and accepted on both platforms**. No `0.6.3-drop2` file and no `feature/0.6.4-m4b-maker` branch ever existed. v0.6.3 therefore closed with Plan 6 and its focused MP3 continuation. *(Superseded status text, kept for the record: this row previously read "Planned; not drafted".)* | Plans 1–3 and 6 |
| 8 | v0.6.4 | `Audiobook Creation Tool v0.6.4 — M4B Maker + M4B Metadata Editor.md` (exact filename, not renamed; the proposed `0.6.4-mp3-and-m4b-metadata-workflows.md` was never created) | **SCOPE SUPERSEDED on 2026-09-13; v0.6.4 = M4B Maker + M4B Metadata Editor in one plan and one branch — COMPLETE through Phase 14 (2026-09-17), Windows accepted 2026-09-15 (Phase 12), macOS accepted 2026-09-15 (Phase 13), NOT merged.** The MP3 half of the old Plan 8 scope had already been delivered and accepted in the v0.6.3 focused MP3 redesign, so it is not part of this drop. Branch `feature/0.6.4-m4b-maker-metadata-editor`, cut from the PR #10 merge `e0bab662b385734807bf264d8f450aebac053dcc` (`--no-track`); fifteen implementation phases plus the documentation closeout, each committed separately (Phase 0 `8b1d3a2` … Phase 13 acceptance `87e1a25`; the Phase 14 checkpoint is the documentation commit that carries this row). **Delivered:** both M4B tools as thin Tk panels over Tk-free workflow / plan / engine / batch modules on the shared multi-Book workspace; the Maker's six Shared / four Book-only fields, Decision 51A naming, non-negative between-track silence, Fast-first with Safe fallback, standard run or explicit custom destination; the Editor's one-file-per-page model, binding preserve-by-default, Shared-starts-blank, the three frozen actions (Save Tags / Clear All Tags (keep chapters) / Remove Series Numbering keeping the Series Name), positional blank-line-preserve chapter edits, no audio re-encode; atomic staged publication with read-back validation on both; success-only series numbering on both through the shared allocator; one `JobController` per batch with Pause/Resume/Cancel and Retry Failed from the frozen plan; one Summary | Detailed log; the M4B artwork service (JPG/PNG native, HEIC/HEIF in memory) and the shared staging module; `Clear All Imports` on all three multi-Book tools (maintainer-directed at Phase 12); the **macOS minimum window raised to 1024×800** by the Phase 13 ruling (Windows unchanged at 920×600). `launcher.TOOLS` still holds exactly six tools. **Evidence:** Phase 11 combined hardening matrix (two real defects found and fixed: ffmetadata escaping, junctions in staging cleanup); Windows Phase 12 on the real launcher — 22 maintainer/conductor rows, 0 defects; macOS Phase 13 on the real launcher — Maker 112 checks / Editor 115 checks, 0 defects after one bounded Mac-only layout remediation (`f9247c4`). Latest gates: Windows (`6cc3eaa`) 7,395 collected / 7,364 passed / 31 skipped / 0 failed; macOS (`f9247c4`) 7,427 / 7,372 / 55 / 0; `verify.py` PASS on both; the Phase 14 Windows gate is recorded in `Handoff.md`. **No separate v0.6.5 Metadata Editor plan exists or is planned.** Version identity remains **`0.6.2`, UNRELEASED**; no tag, release, package, PR or merge. **Phase 15 (retirement of the two temporary plan files and closeout) waits for the maintainer's explicit approval at the Phase 14 gate.** *(Superseded status text, kept for the record: this row previously read "Planned; not drafted", with the temporary drop `0.6.4-mp3-and-m4b-metadata-workflows.md` and the dependency note "Plan 7 validates the shared model first".)* | Plans 1–3 and 6 |
| 9 | *(displaced — see 2026-09-18 note)* | `0.6.5-ui-parity-hardening-release.md` *(proposed filename retained as history; not created)* | *Superseded 2026-09-18 (kept for the record): this cell previously read "Planned; not drafted. **The v0.6.5 assignment — final visual parity, hardening, packaging and release — is unchanged by the 2026-09-13 supersession** of the Plan 7 / Plan 8 split; the passing proposal of 'v0.6.5 = Metadata Editor' was itself superseded and never opened."* **DISPLACED — FUTURE ALLOCATION UNASSIGNED.** The maintainer opened v0.6.5 as a different, unrelated plan (TTS Quality Refinement, `0.6.5-tts-quality-refactor.md`, `feature/0.6.5-tts-quality-refactor`) on the merged v0.6.4 base (PR #11, `1f9bdcf347edaad36d68e237933d3ac6036d0a42`). Plan 9's full scope is preserved and remains approved future work; only its version/release-checkpoint binding is removed pending a separate future maintainer decision. Three panels remain classic on Windows (TTS Audiobook, M4B Converter, Cover Image Resizer). | Plans 1–8 |
...
**Status note (2026-09-17, v0.6.4 M4B Maker + M4B Metadata Editor COMPLETE through Phase 14; pull
request #10 MERGED). This is the current §5 status.** It supersedes the 2026-09-13 note below on two
points: that "pull request #10 is OPEN against `master` and NOT merged" — **PR #10 was merged on
2026-09-13 as `e0bab662b385734807bf264d8f450aebac053dcc`** (parents `83a2bfc7…` and `4b988676…`),
the Plan 6 foundation and the MP3 Tool are integrated, and the feature branch is retained — and that
"Plans 7, 8 and 9 remain undrafted" — **by maintainer ruling on 2026-09-13 the separate Plan 7 (Maker
alone) / Plan 8 (MP3 + Editor) split was superseded**: the MP3 portion had already shipped in v0.6.3,
and the remaining M4B Maker and M4B Metadata Editor adoptions were delivered together as **v0.6.4**
in one temporary plan (`Audiobook Creation Tool v0.6.4 — M4B Maker + M4B Metadata Editor.md`) on one
branch (`feature/0.6.4-m4b-maker-metadata-editor`, cut from the PR #10 merge). That drop is
**COMPLETE through Phase 14**, **Windows accepted (Phase 12, 2026-09-15)** and **macOS accepted
(Phase 13, 2026-09-15)** by the maintainer on the real launcher with real media, and **NOT merged**.
The macOS minimum window is now **1024×800** by the Phase 13 ruling (Windows unchanged). **There is
no separate v0.6.5 Metadata Editor plan; the Plan 9 / v0.6.5 final parity-hardening-release
assignment is unchanged and still undrafted.** The retained Plan-6 drop
`0.6.3-drop1-shared-multi-book-workspace.md` is **eligible for retirement** (all three intended
adopters are on the foundation, the hash gate is empty, the contracts are promoted) but is **not
deleted** — its retirement, and the v0.6.4 plan's, await the maintainer's explicit approval at the
Phase 14 closeout gate; Phase 15 runs only on that approval. Every other fact in the 2026-09-13 note
stands: `0.6.2` remains **UNRELEASED**, the **published GitHub release remains `v0.4.0`**, the
Phase 9 waiver stands, and the literal current tip of `master` is still not asserted here — it is
queried from Git.

**Status note (2026-09-13, Plan 6 foundation and focused MP3 continuation COMPLETE; pull request #10
OPEN). Superseded on 2026-09-17 (see above); kept for the record.** It supersedes the 2026-09-07 note below on exactly one point:
that "Plan 6 has NOT begun … still undrafted … Plans 6–9 all remain unopened". Plan 6 was drafted on
2026-09-07 and then implemented: the shared multi-Book foundation (**Plan 6 Phases 0–8**) and its focused
MP3 continuation (**Phases 1–13**, Windows and macOS acceptance both complete, temporary plan retired at
closeout) are **COMPLETE** on branch `feature/0.6.3-drop1-shared-multi-book-workspace` at checkpoint
`7d3726f273e47f37acf1dd1202e9e6fd13619e5e`, and **pull request #10 is OPEN against `master` and NOT
merged**, awaiting the maintainer's integration decision — see the Plan 6 row above and §15. Every other
fact in the 2026-09-07 note stands: the PRE-PLAN-6 anchor `83a2bfc7de25dbe5a599b48fd73fe306695e490d`,
Plan 5's anchor `7fc9d18b69a2a5b802cc88ef9eada99f17a3df6f`, the Phase 9 waiver, and the release
prohibitions — `0.6.2` remains **UNRELEASED**, the **published GitHub release remains `v0.4.0`**, and the
literal current tip of `master` is still not asserted here. **Plans 7, 8 and 9 remain undrafted.**
...
*Supersession note (2026-09-13, recorded 2026-09-17):* the diagram above is the 2026-07-31 dependency map and is kept as drawn. In execution, **Plan 7 and Plan 8 were replaced by one combined drop** — v0.6.3 delivered Plan 6 plus the MP3 Tool half of Plan 8 (the focused MP3 redesign), and **v0.6.4 delivered the M4B Maker (old Plan 7) and the M4B Metadata Editor (the other half of old Plan 8) together** — so the effective edges are `P6 → v0.6.3 MP3 Tool → v0.6.4 Maker + Editor → P9`. Plan 9's v0.6.5 assignment is unchanged.
...
*Superseded 2026-09-13 (recorded 2026-09-17):* this scope was delivered in full inside the combined **v0.6.4 M4B Maker + M4B Metadata Editor** drop, not as a separate Plan 7 drop or branch. The ownership text above is kept as the record of what that scope was.
...
*Superseded 2026-09-13 (recorded 2026-09-17):* the MP3 Tool half was delivered and accepted in the **v0.6.3 focused MP3 redesign** (with the maintainer's later rulings recorded in `Decisions.md`, 2026-09-12: two actions, blank means blank, Title-derived filenames); the M4B Metadata Editor half was delivered in the combined **v0.6.4** drop together with the M4B Maker, with preserve-by-default binding and Shared starting blank (`Decisions.md`, 2026-09-17). No `0.6.4-mp3-and-m4b-metadata-workflows.md` was ever created.
...
*Displaced 2026-09-18 (recorded at v0.6.5 Phase 0):* this scope is preserved in full and remains approved future work, but it is no longer bound to the "v0.6.5" version identity — the maintainer opened v0.6.5 as the unrelated TTS Quality Refinement plan instead (`0.6.5-tts-quality-refactor.md`, `feature/0.6.5-tts-quality-refactor`, cut from the merged v0.6.4 base at PR #11, `1f9bdcf347edaad36d68e237933d3ac6036d0a42`). Every ownership reference to "Plan 9" elsewhere in this document (Sections 8–10, 14) still stands unchanged; only the version/release-checkpoint number is **DISPLACED — FUTURE ALLOCATION UNASSIGNED** until a separate later maintainer decision assigns one.
...
| Five panels remain classic on Windows | Plan 9. **Now three** — the MP3 Tool (v0.6.3) and the M4B Maker (v0.6.4 Phase 6) were converted by their own plans' explicit scope; TTS Audiobook, M4B Converter and Cover Image Resizer remain classic |
| ttk Combobox popdown and Windows title bar unthemed | Plan 9 review; may remain OS-owned if documented |
| M4B Metadata Editor form scrolls at every size | **DISCHARGED at v0.6.4 Phase 10 (2026-09-14):** the rebuilt Editor has no whole-form canvas; only its chapter box and log scroll, proved at 920×600 on Windows and 1024×800 on macOS. Plan 9 no longer owns this |
...
| Unreadable metadata input may be copied before tag write fails | **CLOSED at v0.6.4 Phase 9 (2026-09-14):** every Editor action copies into private staging and publishes only after the whole edit transaction and a read-back validation succeed; an unreadable source is a visible `SKIPPED_INVALID` Book and never a copy |
...
v0.6.5/Plan 9 assignment — and leaves everything else in that block and every block beneath it
standing as dated history.**

- **v0.6.4 is confirmed MERGED.** Fresh `git fetch origin` showed `origin/master` at
  `1f9bdcf347edaad36d68e237933d3ac6036d0a42`, the merge commit for **pull request #11** merging
  `feature/0.6.4-m4b-maker-metadata-editor` (tip `f5f3926`). The independent read-only
  integration-readiness recheck named in the block below ran and returned READY, and the maintainer
  merged. No tag, package, release, or version bump accompanied the merge; version identity remains
  `0.6.2`, unreleased.
- **v0.6.5 has started — but as a different plan than the one this index previously reserved it
  for.** `feature/0.6.5-tts-quality-refactor` was created from the verified `origin/master` above.
  The active plan is `md-instructions/0.6.5-tts-quality-refactor.md` ("TTS Quality Refinement," v4
  Final), scoped to `scripts/Universal/tts/` audio-quality/UI work. Phase 0 (reorientation, baseline
  synthesis confirmation across Edge/Kokoro/Chatterbox, TTS-path/test audit, v0.5.0 Edge-history
  location) is complete and recorded in `Handoff.md`; no production behavior changed.
- **Plan 9 is DISPLACED — FUTURE ALLOCATION UNASSIGNED**, by explicit maintainer ruling (2026-09-18,
  recorded in `Decisions.md`). Plan 9's full scope (Section 7 above) is preserved intact as approved
  future work; only its "= v0.6.5" version binding is removed. No new version, branch, or plan number
  is invented here — that is a separate future maintainer decision. See Section 7's Plan 9 note and
  the Approved Plan Series Map's matching 2026-09-18 supersession section.
- **Nothing here authorizes starting v0.6.5 Phase 1, drafting Plan 9, or any merge/tag/release.**
...
as open, the Maker and Editor adoptions as "not started and intentionally unscheduled", the next action
as "the maintainer's integration decision: merge pull request #10", and the future work as two separate
branches `feature/0.6.4-m4b-maker` / `feature/0.6.5-m4b-metadata-editor`. Everything else in that block
still stands. Every block below it remains dated history and must not be read as instructions.**

- **Pull request #10 was merged on 2026-09-13** as `e0bab662b385734807bf264d8f450aebac053dcc`
  (parent 1 `83a2bfc7de25dbe5a599b48fd73fe306695e490d`, parent 2
  `4b988676ac258f4f3b4991aa92c99679a2e6ce33`); the Plan 6 foundation and the MP3 Tool are integrated;
  `feature/0.6.3-drop1-shared-multi-book-workspace` is retained at `4b98867`.
- **The maintainer superseded the old split on 2026-09-13**: v0.6.4 = **M4B Maker + M4B Metadata
  Editor in one plan and one branch** (`feature/0.6.4-m4b-maker-metadata-editor`, cut from
  `e0bab662`). Neither `feature/0.6.4-m4b-maker` nor `feature/0.6.5-m4b-metadata-editor` was ever
  created, and no separate v0.6.5 Metadata Editor plan exists. The Plan 9 / v0.6.5 final
  parity-hardening-release assignment is unchanged and still undrafted.
- **v0.6.4 is COMPLETE through Phase 14.** Phases 0–11 implemented and gated (Phase 13's macOS
  remediation `f9247c4` was the last code change); **Windows accepted by the maintainer on 2026-09-15
  (Phase 12, recorded at `17c8330`)**; **macOS accepted by the maintainer on 2026-09-15 (Phase 13,
  recorded at `87e1a25`)**, with the macOS minimum window raised to **1024×800** by ruling; Phase 14
  (this reconciliation) transferred the lasting record to `Decisions.md` (2026-09-17), `Changelog.md`
  (under `[Unreleased]`, **no `[0.6.4]` heading**), `Briefing.md`, `README.md`, `Handoff.md`, this
  index and the Series Map, and re-ran the full gate (recorded in `Handoff.md`). **NOT merged.**
- **THE CURRENT NEXT ACTION IS THE MAINTAINER'S DECISION AT THE PHASE 14 CLOSEOUT GATE: approve or
  reject the retirement of (1) `md-instructions/Audiobook Creation Tool v0.6.4 — M4B Maker + M4B
  Metadata Editor.md` and (2) the retained `md-instructions/0.6.3-drop1-shared-multi-book-workspace.md`.**
  Both are eligible: the v0.6.4 plan is complete and its lasting truth is transferred; the Plan-6 drop's
  three intended adopters are all on the foundation, its hash gate is empty and its contracts are
  promoted. **Neither may be deleted without that explicit approval.** On approval, **Phase 15**
  deletes exactly those two files, re-runs the full gate, commits and pushes one normal closeout
  commit on the feature branch, and stops; the action after that is an independent **READ-ONLY
...
- **Nothing was published and nothing was released.** Version identity remains **`0.6.2`** and
  **UNRELEASED**: no `[0.6.2]`, `[0.6.3]` or `[0.6.4]` changelog heading, **no tag, no GitHub release,
  no package, no `release.py` run, no PR for v0.6.4, no merge**. The **published GitHub release remains
  `v0.4.0`**, and **Plan 9 still owns release hardening and publication**. The PRE-PLAN-6 Phase 9 waiver
  stands. All prior feature and maintenance branches are **retained**. As everywhere else in this file,
  the literal current tip of `master` is not asserted here — it is queried from Git.

**Updated 2026-09-13, at the pre-merge documentation reconciliation for pull request #10. Superseded
on 2026-09-17 (see above); kept for the record. It supersedes the 2026-09-07 block below it on every point that
describes Plan 6 as still ahead of the project: that "IMPLEMENTATION HAS NOT STARTED", that "No phase
has run and no production code, test or configuration exists for it", that "THE CURRENT NEXT ACTION is
PLAN 6 PHASE 0", that the drop is "the single active temporary implementation drop", and that there is
"no pull request". Everything else in that block still stands — the two carried-forward reconciliation
commits, the PRE-PLAN-6 and Plan 5 anchors, the Phase 9 waiver and the release prohibitions. Every block
below it remains dated history and must not be read as instructions.**

- **Plan 6 was implemented after it was drafted, and that implementation is COMPLETE.** The shared
  multi-Book foundation — **Plan 6 Phases 0–8** (book vocabulary, workspace controller, folder-to-book
  construction, shared metadata precedence, frozen book runs, success-only numbering, retry
  dispositions, Tk adapter and developer harness) — is **COMPLETE**. Its focused MP3 continuation — the
  temporary plan `0.6.3-plan6-mp3-tool-redesign.md`, **Phases 1–13**, which rebuilt the MP3 Tool as a
  multi-Book workspace on that foundation — is **COMPLETE**, with **Windows acceptance (Phase 11) and
  macOS acceptance (Phase 12) both complete** by the maintainer on the real launcher with real media,
  and the **focused temporary plan retired after its Phase 13 closeout** with explicit maintainer
  approval. **No Plan 6 or MP3 implementation work is pending.** The lasting record lives in
  `Briefing.md`, `Changelog.md`, `Decisions.md` (2026-09-12) and `Handoff.md`.
- **Where the work is:** branch `feature/0.6.3-drop1-shared-multi-book-workspace`, completed feature
  checkpoint **`7d3726f273e47f37acf1dd1202e9e6fd13619e5e`** before this reconciliation; cut from
  `10a03ed66480d396d65881eea84cb8ae4bfe452e` and still 0 behind the PRE-PLAN-6 anchor
  `83a2bfc7de25dbe5a599b48fd73fe306695e490d`. **Pull request #10 — "Feature/0.6.3 drop1 shared multi
  book workspace" — is OPEN against `master` and is NOT merged.**
- **THE CURRENT NEXT ACTION IS THE MAINTAINER'S INTEGRATION DECISION: merge pull request #10 into
  `master`.** Nothing else comes before it. `master` remains the canonical default branch; nothing is
  created or renamed as `main`.
- **The broader Plan 6 drop `0.6.3-drop1-shared-multi-book-workspace.md` is retained, not retired**,
  because its M4B Maker and M4B Metadata Editor adoptions of the shared workspace have **not started and
  are intentionally unscheduled** under this drop. Retaining the file does not make it active work.
- **After integration, and only then:** the M4B Maker adoption will be **separately planned** on a new
  `feature/0.6.4-m4b-maker` branch cut from the merged `master`; the M4B Metadata Editor adoption will
  follow later on a separate `feature/0.6.5-m4b-metadata-editor` branch, cut only after the 0.6.4 work
  has been independently completed and merged. **Neither branch exists, neither is an active plan, and
  neither is authorized by this note** — each needs its own drop and separate explicit maintainer
  approval before it opens, and a branch name never authorizes a version bump.
- **Nothing was published and nothing was released.** Version identity remains **`0.6.2`** and
  **UNRELEASED**: no `[0.6.2]` and no `[0.6.3]` changelog heading, **no tag, no GitHub release, no
  package, no `release.py` run**. The **published GitHub release remains `v0.4.0`**, and **Plan 9 still
  owns release hardening and publication**. The PRE-PLAN-6 Phase 9 waiver stands (WAIVED / NOT
  APPLICABLE, never a passed test). All prior feature and maintenance branches are **retained**. As
  everywhere else in this file, the literal current tip of `master` is not asserted here — it is queried
  from Git.
...
actions are now complete. Those blocks stay as dated history and must not be read as instructions.**

- **v0.6.2 Plan 5 is fully implemented, reviewed, INTEGRATED and closed.** It merged into `master`
  through **pull request #6** on **2026-09-02** as merge commit
  **`7fc9d18b69a2a5b802cc88ef9eada99f17a3df6f`** (a normal two-parent merge commit; parents
  `81c9c0600ca74a42a22bd09d367a702bee9708fe` and `393a5625f0bce90cb4f9f4313196017acf958f56`). The
  feature branch `feature/0.6.2-m4b-converter-upgrade` is **retained** at `393a5625`.
- **Nothing about Plan 5 is outstanding.** There is no further Plan-5 review, no further Plan-5
  pull request, and no reopening of the implementation. Its temporary drop stays retired.
- **The merge authorized no release.** `0.6.2` is a version identity and remains **unreleased**:
  no `[0.6.2]` changelog heading, **no tag, no GitHub release, no package, no `release.py` run**.
  The published GitHub release remains **`v0.4.0`**, and **Plan 9 still owns release hardening and
  publication**.
- **THE NEXT ACTION is ONE separate bounded PRE-PLAN-6 `Setup_and_Run` / bootstrap self-healing
  remediation.** It addresses the carry-forward defect recorded in `Handoff.md`: an installation with
  an existing `.venv` but a later-missing or broken FFmpeg takes the fast `--launch-only` route,
  which **detects** the missing pair but **cannot provision** one, so re-running the launcher can
  repeat the same state indefinitely. **That remediation is not part of Plan 5 and it is not
  Plan 6.**
- **Plan 6 must NOT begin** — no drafting, no drop, no planning — **until that remediation is
  dispositioned**, and it then still needs separate explicit maintainer approval, as every plan does.

*(Superseded on the points of integration status and next action — the blocks below described, in
turn, a pending integration-readiness recheck, a pending pull request and a pending merge. All three
have since happened: the final recheck returned READY, the PR #6 review returned READY TO MERGE, and
the merge completed on 2026-09-02. They are kept verbatim as dated evidence.)*

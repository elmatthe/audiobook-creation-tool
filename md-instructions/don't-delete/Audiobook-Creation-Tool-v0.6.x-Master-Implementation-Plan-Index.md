...
**Status note (2026-08-22, updated at Plan 4 Phase 15 closeout).** Plans 1 and 2 are merged (pull requests #2 and #3). **Plan 3 and Plan 4 are both complete, approved and closed, and both are awaiting integration review on their own unmerged feature branches** — `feature/0.6.0-drop3-shared-job-controls-importing` and `feature/0.6.1-tts-cover-workflows`. **There is no active temporary implementation drop:** Plan 4's was retired at its closeout, as Plan 3's was before it. `VERSION` is now **`0.6.1`**, set as a version identity at the Plan 4 closeout — there is no tag, release, package or publication, and Plan 9 still owns release. **The nine-plan sequence is unchanged**, no plan was reordered or renamed, no Plan 10 exists, and Decisions 1–55 were not reopened — Decision **52B was partially superseded** by the approved EPUB retirement (its folder-batch half stands; its single-file-EPUB half is withdrawn), which is recorded as a signed ADR in `Decisions.md` rather than as a silent edit to the register. **Plan 5 has not been drafted or started.**
...
**Updated 2026-08-22 at Plan 4 Phase 15 closeout.** Plans 1 and 2 are closed and merged (pull requests #2 and #3). **Plan 3 and Plan 4 are both complete, approved and closed**, and each retired its own temporary implementation document at its own closeout — `md-instructions/0.6.0-drop3-shared-job-controls-importing.md` and `md-instructions/0.6.1-tts-cover-workflows.md`. **There is no active implementation document.**

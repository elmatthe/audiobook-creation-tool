...
| 5 | v0.6.2 | `0.6.2-m4b-converter-upgrade.md` | **ACTIVE — drafted, maintainer-approved, Phase 0 complete; Phase 1 not started.** Branch `feature/0.6.2-m4b-converter-upgrade` from start SHA `81c9c0600ca74a42a22bd09d367a702bee9708fe`. Eighteen phases (0–18). Approved product contracts: **D1** local functional-layout policy with a sanctioned minimal scrolling fallback (`MIN_SIZE` unchanged); **D2** artwork — Preserve keeps, Strip removes, Replace keeps source artwork, no image picker; **D3** direct-file split outputs stay flat per Decision 31A; **D4** split-Preserve inherits only `album`/`artist`/`album_artist` and regenerates `title`/`track`; **D5** three distinct numbering concepts with success-only whole-book allocation; **D6A** whole-book Replace preserves the source chapter map; **D7A** an additive shared `ImportOptions.include_subfolders: bool = True` extension implemented at Phase 7A, plus the Decision 14A `Clear All` label correction. Also fixed: the chapter-start complete-timeline partition, conservative malformed-chapter handling, title flattening before filename sanitisation, an immutable `ConversionPlan` lifecycle, an evidence-driven ffmpeg seek shape, and a deadlock-safe cancellable subprocess contract. `VERSION` stays `0.6.1` until approved closeout. | Plans 1–3 |

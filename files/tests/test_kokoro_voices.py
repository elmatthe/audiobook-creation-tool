...
Kokoro's wheel requires <3.13):
    ...
    Windows:  .venv\\Scripts\\python.exe files\\tests\\test_kokoro_voices.py
    macOS:    .venv/bin/python  files/tests/test_kokoro_voices.py
...
# --- Make scripts/Universal importable so `tts.*` / `shared.*` resolve regardless of cwd.
# This file lives at <repo_root>/files/tests/test_kokoro_voices.py.
_THIS = Path(__file__).resolve()
REPO_ROOT = _THIS.parent.parent.parent
SCRIPTS_ROOT = REPO_ROOT / "scripts" / "Universal"   # the import root
if str(SCRIPTS_ROOT) not in sys.path:
    sys.path.insert(0, str(SCRIPTS_ROOT))

# Import kokoro_synth early: its module-load HF_HOME redirect points the model
# cache at files/runtime-data/models/huggingface/ before kokoro is ever imported.
...
TEST_LOGS_DIR = REPO_ROOT / "files" / "test-logs" / "kokoro-voices"
...
# ---------------------------------------------------------------------------
def main() -> int:
    print(f"Project tree : {REPO_ROOT}")

"""M4B Metadata Editor batch execution, success numbering and Retry Failed —
v0.6.4 Phase 9.

One frozen :class:`~mp3_tools.m4b_metadata_plan.RunPlan` runs through the
shared job architecture here, with no panel involved — the Maker's Phase 5
runner, re-composed for the Editor's three actions. The panel (Phase 10) only
wires buttons to it and drains its events.
...
- **One ``JobController`` and one ``JobReporter`` per batch attempt**, never one
  ...
  the Phase 9 engine, between steps inside a Book.
...
- **The Phase 9 engine** for every Book in frozen order:
  :func:`~mp3_tools.m4b_metadata_processing.stage_book` then
  :func:`~mp3_tools.m4b_metadata_processing.publish_book`, with the shared
...
- **The Plan 3 result vocabulary.** Each Book settles into its own
  ...
  while its source could still be retried records one retryable
  ``FailureRecord`` for its **real** occurrence; a Book that could not start at
  all (unusable artwork, a vanished source) records one item-less fatal
  failure. No identity is ever invented. The batch composes into the Plan 6
  ``WorkspaceRunResult``; Books the plan skipped as invalid keep that frozen
  disposition; and Retry Failed is ``retry_failed_books`` on the result.

Success-only Series Part numbering (plan section 6.9)
-----------------------------------------------------
With Auto-number off there is no allocator: a Save preserves the source's
Series Part unless the Book edits it, a Clear leaves none. With it on (Save or
Clear — Remove Series Numbering never numbers and the plan already says so),
:class:`EditorRun` owns one ``SuccessNumbers`` counter from the frozen Start
Part — **execution state, not plan state** — for the whole retry chain. For
each Book that staged and validated: ``propose()``, write the proposed part
onto the **staged** copy with the shared writer, validate again, publish, and
only then ``commit()``. A failure before or during publication consumes
nothing; a publication failure retains the validated candidate so the retry
can reuse it, and the retry rewrites that stale tentative part to its own
newly proposed number before publishing. Describing a retry consumes nothing.

Retry Failed
------------
:meth:`EditorRun.retry_failed` builds a new attempt at the **same** run — same
...
from mp3_tools.m4b_metadata_plan import BookPlan, EditorAction, RunPlan
...
#: The action names the reporter uses, one per Editor action.
ACTION_LABELS: Mapping[EditorAction, str] = {
    EditorAction.SAVE_TAGS: "Save Tags",
    EditorAction.CLEAR_ALL_TAGS: "Clear All Tags",
    EditorAction.REMOVE_SERIES_NUMBERING: "Remove Series Numbering",
}

#: Stages at which a failure is the Book's source's to retry. Anything else is
#: a fatal, item-less failure: the plan itself cannot be run as it stands.
_RETRYABLE_STAGES = frozenset({
    "copy", "clear", "tags", "cover", "chapters", "series", "validate", "publish",
    "processing", "staging", "number",
})
...
class Attempt:
    ...
    def __init__(self, owner: "EditorRun", *, number: int, run_id: str,
    ...
    def _execute(self, records: list[BookRecord]) -> None:
        ...
        for done, book in enumerate(self.books):
            ...
            self.reporter.progress(done, total, item_id=book.occurrence_id,
    ...
    def _run_book(self, book: BookPlan) -> BookRecord:
        ...
        def failed(stage: str, message: str, detail: str) -> BookRecord:
            if stage in _RETRYABLE_STAGES:
                records = (FailureRecord(item_id=book.occurrence_id, stage=stage,
                                         display_message=message,
                                         technical_detail=detail or message, retryable=True,
                                         snapshot_id=snapshot_id),)
                reporter.failure(message, detail, item_id=book.occurrence_id, stage=stage)
            else:
        ...
        # A retry reuses a retained, validated candidate rather than re-copying.
        ...
        if not retained:
            outcome = proc.stage_book(book, work_root=work_root, checkpoint=self.checkpoint,
                                      on_event=on_event)
        ...
        result = RunResult.settle(book.snapshot, completed_ids=(book.occurrence_id,))
        return BookRecord(book_id=book.book_id, result=result, published=published, part=part)


def _part_on(staged: Path) -> int | None:
    """The tentative Series Part a retained candidate still carries, if numeric."""
    try:
        value = str(metadata.read_m4b_tags(staged).get("series_part", "") or "").strip()
    except Exception:  # noqa: BLE001 - validation decides; this only describes
        return None
...
class EditorRun:
    ...
    def __init__(self, plan: RunPlan, *, id_factory: IdFactory,
        ...
        if not isinstance(plan, RunPlan):
            raise BatchError(f"plan must be an Editor RunPlan, got {type(plan).__name__}")
        ...
        # The plan already says Remove Series Numbering never numbers.
    ...
    def _begin(self, books: tuple[BookPlan, ...], *, retry) -> Attempt:
        ...
        run_id = self._run_id
        item_ids = tuple(book.occurrence_id for book in self.plan.books)
        ...
        controller.start()
        label = "Retry Failed" if retry is not None else ACTION_LABELS[self.plan.action]
        reporter.stage_changed(
            STAGE_PREPARE,
            f"{label}: {len(books)} Book(s) → {self.plan.run_directory}")
        reporter.output_location(self.plan.run_directory, f"Output: {self.plan.run_directory}")
    ...
    def _prune_work_root(self) -> None:
        ...
        m4b_staging.prune_work_root(self.plan.work_root)

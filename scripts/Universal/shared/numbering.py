...
Three different numbers appear in a multi-book run and Decision 5 / Decision 21A
keep them apart. Two of them are **structural**: a split output's filename prefix
and its ``track`` are both the segment's position inside its own book, frozen into
the run before anything is written, identical on every attempt, and untouched by
whether any other book succeeded. Neither of them lives here.

The third does live here, and it is the only one that interacts with success: the
sequential number a **completed book** carries. Decision 21A says it must not be
pre-assigned, and 28A says the result must have no gaps -- so if the second of
three books fails, the third is number **2**, not 3.

**Why the API is shaped like this.** The number has to exist *before* the work
runs, because it is encoded into the output; but it must not count until the output
actually exists. Those are two different events, so they are two different calls.
:meth:`SuccessNumbers.propose` reads the next value and advances nothing -- asking
is free, and asking twice is the same answer. :meth:`SuccessNumbers.commit` is the
only thing that moves the counter, it takes the token it is confirming, and it
refuses one that has already been spent. So "forgot to commit" costs a number
nothing, and "committed twice" is an error rather than a silent skipped number.

**Why this state is never in a frozen plan.** A run snapshot is immutable and is
what a retry re-reads; a success counter is the opposite of that -- it is a fact
about one attempt's execution, discovered while it runs. Keeping it out is what
lets a plan stay retry-stable.

**Promoted, not rewritten.** v0.6.3 Plan 6 Phase 6 moved this module here from
``mp3_tools/m4b_numbering.py`` so one allocator serves every consumer;
``mp3_tools.m4b_numbering`` is now a thin re-export of these same objects. The
declarations below are unchanged, and a test proves that mechanically against the
pre-move source. The allocator still has **no knowledge of what a book is**.

Pure: integers in, integers out. No Tk, no filesystem, no ffmpeg, no metadata
vocabulary, no controller, no paths, no threads and no clock.
"""

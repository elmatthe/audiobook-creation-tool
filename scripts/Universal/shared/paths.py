...
Layout (repo root)::
    ...
    <repo_root>/
        ...
        files/
            ...
            runtime-data/
                logs/
                models/           <- HuggingFace (Kokoro) cache
                settings.json
"""
...
# This file lives at <repo_root>/scripts/Universal/shared/paths.py
_THIS = Path(__file__).resolve()

SHARED_DIR: Path = _THIS.parent
SCRIPTS_DIR: Path = SHARED_DIR.parent          # scripts/Universal — the import root
REPO_ROOT: Path = SCRIPTS_DIR.parent.parent

TTS_DIR: Path = SCRIPTS_DIR / "tts"
MP3_TOOLS_DIR: Path = SCRIPTS_DIR / "mp3_tools"

FILES_DIR: Path = REPO_ROOT / "files"
RESOURCES_DIR: Path = FILES_DIR / "runtime-data"
LOGS_DIR: Path = RESOURCES_DIR / "logs"
BIN_DIR: Path = FILES_DIR / "bin"
SETTINGS_FILE: Path = RESOURCES_DIR / "settings.json"

REQUIREMENTS_FILE: Path = REPO_ROOT / "scripts" / "requirements.txt"

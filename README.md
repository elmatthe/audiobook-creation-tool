...
**Non‑destructive by design:** every tool that transforms a file works on a **copy**. Your imported originals are only ever read, never modified, renamed or deleted. If two imported files share a name, the second output is numbered (`Book.m4b`, `Book-1.m4b`) rather than replacing the first.
